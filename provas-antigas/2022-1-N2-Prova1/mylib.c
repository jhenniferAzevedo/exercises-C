...
void read_data(student_data student[][MAX_TURMAS], class_data class[]) {
    for (int n_class = 0; n_class < MAX_TURMAS; n_class++) {
        int total_students;
        printf("\nTURMA %d\n", n_class + 1);
...
void read_student_data(student_data student[][MAX_TURMAS], int n_class, int total_students) {
    for (int n_student = 0; n_student < total_students; n_student++) {
        printf("\n\tALUNO %d\n", n_student + 1);
...
void read_grades_by_student(student_data student[][MAX_TURMAS], int n_student, int n_class) {
    for (int n_grade = 0; n_grade < MAX_NOTAS; n_grade++) {
...
void counts_substitute_test(student_data student[][MAX_TURMAS], class_data class[]) {
    for (int i = 0; i < MAX_TURMAS; i++) {
        ...
        for (int j = 0; j < class[i].total_alunos; j++) {
            int status = check_student_status(student, j, i);
            if (status == 3) {
...
void shows_query_per_student(student_data student[][MAX_TURMAS], class_data class[])
{
    ...
    n_student--;
    n_student--;
    show_stutent_data(student, n_student, n_class);
    system("pause");
}
