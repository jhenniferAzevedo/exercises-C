...
int main(void)
{
    char cor[QNT_UN][QNT_ST][TAM];
    char tecido[QNT_UN][QNT_ST][TAM];

    int funcionariosUN[QNT_UN];
    int funcionariosST[QNT_UN][QNT_ST];
    int corCamisa[QNT_UN][QNT_ST];
    int tecidoCalca[QNT_UN][QNT_ST];

    int unidade, setor, relatorio;
    int camisaB = 0, camisaP = 0, camisaA = 0, calcaM = 0, calcaT = 0;
    int maiorUN = 0, indiceMaiorUN;

    for (unidade = 0; unidade < QNT_UN; unidade++)
    {
        funcionariosUN[unidade] = 0;
    }
    ...
    {
        printf("\n* UNIDADE %d *\n", unidade + 1);
        for (setor = 0; setor < QNT_ST; setor++)
        {
            printf("\n\tSETOR %d\n", setor + 1);
            ...
            case 1:
                ...
                camisaB += funcionariosST[unidade][setor] * POR_FUNC;
                break;
            case 2:
                strcpy(cor[unidade][setor], "preta");
                camisaP += funcionariosST[unidade][setor] * POR_FUNC;
                break;
            case 3:
                strcpy(cor[unidade][setor], "azul");
                camisaA += funcionariosST[unidade][setor] * POR_FUNC;
                break;
            }
            ...
            case 1:
                ...
                calcaM += funcionariosST[unidade][setor] * POR_FUNC;
                break;
            case 2:
                strcpy(tecido[unidade][setor], "tectel");
                calcaT += funcionariosST[unidade][setor] * POR_FUNC;
                break;
            }
